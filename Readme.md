...
* **[Event Handling & Pub/Sub Patterns](./Concepts/Event_Handling_and_PubSub_Patterns.md)**: Event delegation & bubbling strategies, debounce, throttle, requestIdleCallback, custom typed event bus / pub-sub for decoupled communication, AbortController for cancellable requests & listener cleanup, BroadcastChannel for cross-tab communication, and real-world architecture patterns (search, infinite scroll, auto-save).

* **[Micro-Frontends & Module Federation](./Concepts/Micro_Frontends_and_Module_Federation.md)**: Monolith vs Micro-Frontend trade-offs, MFE integration approaches (build-time, iframes, JS dynamic remotes, Web Components, Module Federation, server-side composition), App Shell pattern for hosting multiple MFEs under one UI, cross-MFE communication (Custom Events, Typed Event Bus, Props, URL state, BroadcastChannel), Webpack 5 Module Federation deep dive (host/remote config, shared deps, async boundary, dynamic remotes, bi-directional federation), MF 2.0 & Vite support, routing strategies, deployment & CI/CD, and real-world examples (IKEA, Spotify, Zalando, Amazon).

* **[Logging, Analytics & Feature Flags](./Concepts/Logging_Analytics_and_Feature_Flags.md)**: Analytics architecture (event taxonomy, batching with sendBeacon, funnel tracking), A/B testing infrastructure (client-side vs server-side, flicker avoidance, statistical significance), feature flag systems (LaunchDarkly, Unleash — architecture, targeting rules, flag hygiene), session replay & heatmaps (rrweb, Hotjar, Clarity, privacy/PII), frontend error tracking & debugging (global handlers, Sentry setup, source maps, Error Boundaries, breadcrumbs), and full logging pipeline (client-side collection, log levels & sampling, transport via sendBeacon/IndexedDB, storage with ELK/Loki/Datadog/ClickHouse).
